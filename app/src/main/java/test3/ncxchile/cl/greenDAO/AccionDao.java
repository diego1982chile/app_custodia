...
public class AccionDao extends AbstractDao<Accion, Long> {
    ...
    */
    ...
    public List getAccionesSinMapa(){
        List acciones= queryBuilder()
                .where(Properties.IdMapa.isNull(),Properties.Nombre.notEq("Buscar Acta"))
                .list();
        return acciones;
    }

    public List getAccionesByTarea(Long idTarea){
        List acciones= queryBuilder()
                .where(Properties.IdTarea.eq(idTarea),Properties.Nombre.notEq("Buscar Acta"))
                .list();
        return acciones;
    }

    public void discardAcciones(Long idTarea){
        final List accionesNoVigentes=queryBuilder()
                .where(Properties.IdTarea.eq(idTarea))
                .list();

        Global.daoSession.runInTx(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < accionesNoVigentes.size(); ++i) {
                    Accion accion = (Accion) accionesNoVigentes.get(i);
                    accion.setSincronizada(true);
                    update(accion);
                    refresh(accion);
                }
            }
